...
size_t get_sha1_digest_length(void)
{
...
string s3fs_sha1_hex(const unsigned char* data, unsigned int datalen, unsigned char** digest, unsigned int* digestlen)
{
	(*digestlen) = static_cast<unsigned int>(get_sha1_digest_length());
	if(NULL == ((*digest) = reinterpret_cast<unsigned char*>(malloc(*digestlen)))){
	  return false;
	}
	
	struct sha1_ctx ctx_sha1;
	sha1_init(&ctx_sha1);
	sha1_update(&ctx_sha1, datalen, data);
	sha1_digest(&ctx_sha1, *digestlen, *digest);

	return s3fs_hex(*digest, *digestlen);
}
...
string s3fs_sha1_hex(const unsigned char* data, unsigned int datalen, unsigned char** digest, unsigned int* digestlen)
{
  ...
  gcry_md_hd_t   ctx_sha1;
  gcry_error_t   err;
  if(GPG_ERR_NO_ERROR != (err = gcry_md_open(&ctx_sha1, GCRY_MD_SHA1, 0))){
    S3FS_PRN_ERR("SHA1 context creation failure: %s/%s", gcry_strsource(err), gcry_strerror(err));
    free(*digest);
    return false;
  }
  gcry_md_write(ctx_sha1, data, datalen);
  memcpy(*digest, gcry_md_read(ctx_sha1, 0), *digestlen);
  gcry_md_close(ctx_sha1);
